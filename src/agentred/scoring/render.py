"""The page an operator reads, rendered from a finished analysis.

A pure function of the analysis file. It calls no model and reaches no store, which is the
whole point of the split: the analysis is paid for once and this can be rebuilt a hundred
times while somebody argues about the wording. A page that costs money to render is one that
gets revised twice and then abandoned, and what a merchant reads ends up being the draft
nobody could afford to improve.

It opens with money and with the merchant's own rules. Methodology, sample size and how much
of the verdict rests on a model rather than on a tool-call log are all here, at the bottom,
where they belong for this reader and nowhere else.
"""
...
.provenance { color:var(--muted); font-size:.85rem; margin:-1.4rem 0 2.2rem; }
...
def provenance_line(analysis: dict[str, Any]) -> str:
    """Say which runs these numbers are from, and which agent version they are valid for.

    A scorecard is valid for one (config, policy, model, tool) version tuple, and change any
    of the four and the agent is untested again. A page that omits the tuple reads as a
    statement about an agent rather than about one version of one, which is exactly the
    misreading that lets a stale result outlive the thing it measured.

    Args:
        analysis: A finished analysis.

    Returns:
        A paragraph of HTML, or empty when the analysis predates run identity being recorded.
    """
    runs = analysis.get("runs") or []
    if not runs:
        return ""
    named = ", ".join(escape(run["notes"] or run["run_id"]) for run in runs)
    targets = sorted({run["target"] for run in runs})
    tuples = sorted(
        {
            "config {config} / policy {policy} / model {model} / tools {tools}".format(
                **run["versions"]
            )
            for run in runs
        }
    )
    versions = "; ".join(escape(t) for t in tuples)
    plural = "s" if len(runs) > 1 else ""
    return (
        f'<p class="provenance">From {named} against '
        f"<strong>{escape(', '.join(targets))}</strong>. "
        f"Valid for {versions}. Change any of those four and the agent{plural} "
        "below is untested again.</p>"
    )
